...
DECISIONS = ROOT / 'rebuild-2026-09-17' / 'REBUILD_DECISIONS.md'
BOUNDARIES = ROOT / 'rebuild-2026-09-17' / 'EVIDENCE_BOUNDARIES.md'
...
def main() -> None:
    ...
    for path, label in [
        (TARGET, 'delegates rebuild deck'),
        (SCRIPT, 'complete speaker script'),
        (DECISIONS, 'rebuild decisions'),
    ...
    for marker in [
        ...
        'No claim that one contract models every conceivable type system',
        'No invented performance numbers',
        'No novelty claim for language composition',
    ]:
        assert marker in boundaries, 'evidence boundary missing: ' + marker
    ...
    print(
        f'Delegates rebuild deck PASS: {len(main_keys)} main + {len(support_keys)} support slides; '
        'required narrative moments, speaker script, evidence boundaries and validation record checked'
    )
