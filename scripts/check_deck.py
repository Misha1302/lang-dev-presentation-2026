...
assert m1 and m2, 'speaker notes objects missing'
notes_by_key=json.loads(m1.group(1)); notes_by_key.update(json.loads(m2.group(1)))
...
full=re.sub(r'\s+',' ',html+' '+js1+' '+js2); missing=[x for x in required if x not in full]; assert not missing, f'missing narrative/evidence elements: {missing}'
...
assert 'appendixBtn' in html and 'speaker-notes-1.js' in html and 'speaker-notes-2.js' in html
